...
- The Pinned Runtime lifecycle is instance-owned: the application host (or CLI command) creates a `PinnedRuntime`, authorizes through it, passes it to the indexing pipeline, and closes that same instance on shutdown. Each `PinnedRuntime` owns its inference scheduler, embedding backend, and llama-server process; closing the runtime stops them. A process-wide lock still serializes inference so at most one llama-server request runs at a time.
