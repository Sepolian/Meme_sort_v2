...
from .semantic_retrieval import (
    blob_to_vector as _semantic_blob_to_vector,
    rank_asset_vector_rows,
    scan_duplicate_vector_rows,
    vector_to_blob as _semantic_vector_to_blob,
)
...
def _mark_job_running(conn: sqlite3.Connection, job_id: str) -> None:
    job_queue.mark_job_running(conn, job_id)


def _mark_job_completed(conn: sqlite3.Connection, job_id: str) -> None:
...
def _mark_job_failed(
    ...
    job_queue.mark_job_failed(conn, job_id, error_code, error_detail)
...
def _run_embed_asset_job(
    ...
    instruction = _instruction_text_for_key(str(recipe_row["instruction_key"]))
    ...
    if str(payload.get("media_type")) == "image/gif":
        frame_payloads = _extract_gif_frame_bytes(
            image_bytes,
            preprocess_version,
        ...
        processed_image_bytes = _preprocess_image_bytes(
            image_bytes,
            preprocess_version,
        )
        vector = backend.embed_image_bytes(
...
def _vector_to_blob(vector: np.ndarray) -> bytes:
    return _semantic_vector_to_blob(vector)


def _blob_to_vector(blob: bytes, vector_dim: int) -> np.ndarray:
    return _semantic_blob_to_vector(blob, vector_dim)
...
def _rank_vector_rows(
    ...
    return rank_asset_vector_rows(query_vector, vector_rows, top_k)
